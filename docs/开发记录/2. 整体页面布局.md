...
在 MainLayout 中设置上下布局

- `Header`
- `<main>`：其中使用`<Outlet />`展示子路由
...
1. CSS 中响应：最常见，利用 Tailwind CSS 提供的断点，一般 `md` 就是手机端了。  
   故可以使用`hidden md:block`来在桌面端显示、`block md:hidden`在移动端显示。
2. JS 中响应：采用`useIsMobile` hook 判断是否为移动端，然后根据不同的设备展示不同的布局。

```bash
npx usehooks-cli@latest add use-is-mobile
```

## 3. 登录页：采用 shadcn/ui 的登录 Block
...
后续就准备正式开始产品页面，以完成基本功能。

## 踩坑

### 1. JSX 与 Component 的区别

虽然大多数情况下两者作为属性传入都可以（`trigger={<Component />}` / `trigger={<div />}`），  
但实际上逻辑有很大不同，尤其是在 shadcn/ui 中。

shadcn/ui 会通过 `asChild`，将 props（比如`onClick`等）向下传递，  
最典型如`<DropdownMenuTrigger />`，如果自定义子节点，需要用`asChild`将`onClick`处理传到子节点中，否则点击并不会打开菜单。

- 如果是 JSX 语法：则会正确传递
- 如果是 Component：如果 Component 没有解析`...props, ref`并向下传递的话，则会造成上面的问题

所以建议用非 Component 的 JSX。
