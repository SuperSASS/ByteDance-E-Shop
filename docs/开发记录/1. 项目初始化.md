...
本 Task 将从 0 开始（完全的新建文件夹），创建出一个基于 Monorepo 组织方式、采用 React 18 + TS + Vite + Shadcn UI + Tailwind CSS + i18next + React Router + 基本 Vitest, Zustand 的 Hello World 项目。
...
3. 配置底座
   ...
   2. 路由：React Router
   3. 国际化：react-i18next
4. 配置辅助工具和仓库：ESLint + Prettier + Husky + lint-staged，初始化仓库并提交
5. 配置基本测试框架：Vitest
6. 配置基本状态管理框架：zustand
        ...
        return (
          <>
            <div className="flex flex-col items-center justify-center h-screen">
              ...
              <Button className="mb-4" onClick={() => i18n.changeLanguage(i18n.language === 'zh' ? 'en' : 'zh')}>{t('switchLanguage')}</Button>
            </div>
          </>
        )
      }
...
## 4. 配置辅助工具和仓库: ESLint + Prettier + Commitlint
...
## 5. 组件单元测试: Vitest + React Testing Library + jsdom
...
2. 在`src/stores`目录下创建“用户验证”状态`useAuthStore.ts`：
...
3. 添加“登录”页面，模拟登录
4. 配置路由，目前先简单实现“当未登录时，跳转到登录页”（日后可能根据实际需求，只对部分功能需要登录）

## 杂项

### 1. 支持主题切换

参考：[Shadcn UI - Theme](https://ui.shadcn.com/docs/overview/theme)实现即可。
